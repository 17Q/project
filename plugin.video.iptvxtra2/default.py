...
vod_url      = '%s:%s/enigma2.php?username=%s&password=%s&type=get_vod_streams&cat_id='%(host,user.port,username,password)
...
fav_home = xbmc.translatePath(os.path.join('special://profile/addon_data/'+user.id,'fav_list.txt'))
if not os.path.exists(fav_home):
    f = open(fav_home, mode='w')
    f.write('>IPTVXtra2<\n')
    f.close()
...
def start():
    ...
    if username=="":
        ...
        control.setSetting('continue','true')
        addSrc()
        xbmcgui.Dialog().ok(user.name, 'Addon must now be restarted to continue 1st run setup.')
        sys.exit()

    elif control.setting('continue')== 'true':
        control.setSetting('continue','false')
        vodcatselect()
        getchan('0')
        correctPVR('0')
        home()
...
def vodcatselect():
    ...
    if (xbmcaddon.Addon().getSetting('hidexxx')=='true'):
        dialog = xbmcgui.Dialog().multiselect('Select VOD Categories to Update', ['Movies','Classics','Peliculas','Clasicos','Foreign','TV Series','Series de TV','Pay-per-View'])
    else:
        dialog = xbmcgui.Dialog().multiselect('Select VOD Categories to Update', ['Movies','Classics','Peliculas','Clasicos','Foreign','TV Series','Series de TV','Pay-per-View','Adult XXX'])
    if 0 in dialog:
        vod('23')
    if 1 in dialog:
        vod('27')
...
def vod(cat):
    ...
    vodlst = "{\n"
    last = "last"+str(cat)
    top = xbmcaddon.Addon(user.id).getSetting(id=last)
    itop = int(top)
    hi = itop
    ...
    for a in all_cats:
        try:
            ...
            catID = tools.regex_from_to(a,'<category_id>','</category_id>')
            ...
            line = '["name":"'+name+'","stream_id":"'+vodID+'","stream_icon":"'+thumb+'","plot":"'+plot+'","url":"'+url+'","runt":"'+runt+'","genre":"'+genre+'","year":"'+year+'","cast":"'+cast+'","dir":"'+dir+'","cat":"'+catID+'","new":"'
    ...
    xbmcaddon.Addon(user.id).setSetting(id=last, value=str(hi))
    saveVod(vodlst,cat)

    catname = ['Movies','Classics','Peliculas','Clasicos','Foreign','TV Series','Series de TV','Adult XXX']
    catnum  = ['23','27','21','28','26','24','22','25']

    catconv = catname[catnum.index(cat)]
    xbmc.executebuiltin("Notification(IPTVXtra II,[I][COLOR lime]    . .*VOD -"+catconv+" Category Updated.[/COLOR][/I],1000)")
...
def fav(chanlist):
    ...
    try:
        fileObject = open((basePath+'fav_list.txt'),'r')
        for a in fileObject:
            favname = tools.regex_from_to(a,'>','<')
            preslct.append(chanlist.index(str(favname)))
#            preslct.append(chanlist.index(str(a)))

    except: pass

    fav = ""
    dialog = xbmcgui.Dialog().multiselect('Select Favorites', chanlist, preselect = preslct)
    for item in dialog:
        name = chanlist[item]
        line = '>'+name+'<\n'
        fav += line

    chanData = open((basePath+'fav_list.txt'),'w')
    chanData.write(fav)
    chanData.close()
    xbmc.executebuiltin("Notification(IPTVXtra II,[I][COLOR lime]    . .*LiveTV Favorites Updated.[/COLOR][/I],2000)")
    getchan('0')
...
def readVod(cat):
    ...
    mod=''
    if (cat !='ppv'):
        dialog = xbmcgui.Dialog().select('Select type of list?', ['New items only *(Since last update)','Partial list (Last x number of items)','Complete list','Search'])
        if dialog==0:
            mod ='1'
        elif dialog==1:
            mod ='0'
            maxcnt = xbmcgui.Dialog().numeric(0, 'Enter # of most recent items to list')
            maxcnt = int(maxcnt)
        elif dialog==2:
            mod ='0'
        elif dialog==3:
            mod ='2'
            srch = xbmcgui.Dialog().input('Enter search term')
        else: pass

    dialog = xbmcgui.Dialog().select('Sort List by ?', ['Date added *(Newest to Oldest)','Title','Genre','Release Date'])
    if dialog==0:
        sortby = 0
    elif dialog==1:
        sortby = 9
...
#    all_chans = tools.regex_get_all(fileObject,'{"num":',':0},')
    for a in fileObject:
        ...
        plot = tools.regex_from_to(a,'plot":"','",')
        url = tools.regex_from_to(a,'url":"','",')
        runt = tools.regex_from_to(a,'runt":"','",')
        genre = tools.regex_from_to(a,'genre":"','",')
        year = tools.regex_from_to(a,'year":"','",')
        ratin = tools.regex_from_to(a,'ratin":"','",')
        cast = tools.regex_from_to(a,'cast":"','",')
        dir = tools.regex_from_to(a,'dir":"','",')
        catm = tools.regex_from_to(a,'cat":"','",')
        ...
        if mod=='1':
            if new =='true':
                if not (name==""):
                    tools.addDirMeta(str(name)+'  *New!',url,4,thumb,thumb,plot,year,cast.split(", "),ratin,runt,genre,dir)
            else: pass
            ...
            if not (name==""):
                if new =='true':
                    ...
                    tools.addDirMeta(str(name),url,4,thumb,thumb,plot,year,cast.split(", "),ratin,runt,genre,dir)
                    cnt += 1
    ...
    xbmcplugin.addSortMethod(int(sys.argv[1]), sortby)
...
def readacct():
    ...
    fileObject = open((basePath+'vod-act.txt'),'r')
    for a in fileObject:
        useracct = tools.regex_from_to(a,'"user":"','"')
        passacct = tools.regex_from_to(a,'"pass":"','"')
        status = tools.regex_from_to(a,'"status":"','"')
        connects = tools.regex_from_to(a,'"maxconn":"','"')
        active = tools.regex_from_to(a,'"actconn":"','"')
        serverurl = tools.regex_from_to(a,'"server":"','"')
        expiry = tools.regex_from_to(a,'"acctexp":"','"')
        ip = tools.regex_from_to(a,'"locip":"','"')
        extip = tools.regex_from_to(a,'"extip":"','"')
#    "user":"'+useracct+'","pass":"'+passacct+'","status":"'+status+'","maxconn":"'+connects+'","actconn":"'+active+'","server":"'+serverurl+'","acctexp":"'+expiry+'","locip":"'+ip+'","extip":"'+extip+']\n'

        text = "[B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR][CR][B]%s[/B]%s[CR]" % ('Username:  ',useracct,'Server URL:  ',serverurl,'Expiry Date:  ',expiry,'Account Status:  ',status,'Current Connections:  ',active,'Allowed Connections:  ',connects,'External IP Address:  ',extip)
        xbmcgui.Dialog().textviewer('Account Information', text)
...
def accountinfo():
    ...
    try:
        ...
        for a in all_chans:
            ...
            if ('PPV' in name):
                ...
                line = '["name":"'+name+'","stream_id":"'+vodID+'","stream_icon":"'+thumb+'"],\n'
                Ppv += line
    except:
        pass
    ...
    saveVod(Act,'act')
    saveVod(Ppv,'ppv')
    xbmc.executebuiltin("Notification(IPTVXtra II,[I][COLOR lime]    . .*Pay-per-View Updated.[/COLOR][/I],1000)")
...
def extras():
    ...
    tools.addDir('Add/ Edit Live TV Favorite Channels','url',22,'',icon_set,fan_set,'')
