...
def skin_location():
	return translate_path('special://home/addons/plugin.video.david')
...
def playback_check_success():
	return get_setting('playback.check_success', 'true') == 'true'

def playback_success_timeout():
	return int(get_setting('playback.success_timeout', '20')) * 10
...
def download_directory(media_type):
	return translate_path(get_setting({'movie': 'movie_download_directory', 'episode': 'tvshow_download_directory', 'thumb_url': 'image_download_directory',
									'image_url': 'image_download_directory', 'image': 'image_download_directory', 'tools': 'premium_download_directory',
...
def autoplay_next_settings():
	...
	return {'scraper_time': scraper_time, 'threshold': threshold, 'run_popup': run_popup, 'window_percentage': window_percentage}
...
def auto_start_david():
	return get_setting('auto_start_david') == 'true'
...
def nextep_content_settings():
	...
	sort_key = 'david_last_played' if sort_type == 0 else 'david_first_aired' if sort_type == 1 else 'david_name'
...
def get_art_provider():
	...
	return {True: ('poster2', 'poster', 'fanart2', 'fanart', 'clearlogo2', 'clearlogo'),
			False: ('poster', 'poster2', 'fanart', 'fanart2', 'clearlogo2', 'clearlogo')}[get_setting('fanarttv.default') == 'true']
