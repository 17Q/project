...
database, ls, WATCHED_DB, TRAKT_DB = kodi_utils.database, kodi_utils.local_string, kodi_utils.watched_db, kodi_utils.trakt_db
indicators_dict = {0: WATCHED_DB, 1: TRAKT_DB}

def get_database(watched_indicators=None):
	return indicators_dict[watched_indicators or settings.watched_indicators()]
...
def batch_erase_bookmark(watched_indicators, insert_list, action):
	try:
		...
		if watched_indicators == 1:
			...
			kodi_utils.Thread(target=_process).start()
		dbcon = make_database_connection(get_database(watched_indicators))
		dbcur = set_PRAGMAS(dbcon)
...
def mark_as_watched_unwatched_movie(params):
	...
	watched_indicators = settings.watched_indicators()
	if watched_indicators == 1:
		if from_playback == 'true' and trakt_official_status(media_type) == False: kodi_utils.sleep(3000)
...
def refresh_container(refresh=True):
	if refresh: kodi_utils.widget_refresh()

def clear_local_bookmarks():
	try:
		dbcon = make_database_connection(kodi_utils.get_video_database_path())
		dbcur = set_PRAGMAS(dbcon)
		file_ids = dbcur.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.david%'").fetchall()
