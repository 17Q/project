...
json, Thread, ls, urlparse, parse_qsl = kodi_utils.json, kodi_utils.Thread, kodi_utils.local_string, kodi_utils.urlparse, kodi_utils.parse_qsl
confirm_progress_media, poster_empty, get_setting = kodi_utils.confirm_progress_media, kodi_utils.empty_poster, kodi_utils.get_setting
...
class Downloader:
	...
	def get_filename(self):
		if self.final_name: final_name = self.final_name
			...
			name_url = kodi_utils.unquote(self.url)
	...
	def finish_download(self, title, media_type, downloaded, image):
		...
		if self.media_type == 'image_url':
			...
			if not downloaded or not playing: 
				kodi_utils.ok_dialog(text=text, top_space=False)
	...
	def confirm_download(self):
		...
		if self.action not in ('image', 'meta.pack'):
			...
			else: choice = kodi_utils.confirm_dialog(text=text, top_space=False)
		return choice
