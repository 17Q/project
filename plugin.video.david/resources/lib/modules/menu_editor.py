...
build_url, confirm_dialog, dialog, sleep, container_refresh = kodi_utils.build_url, kodi_utils.confirm_dialog, kodi_utils.dialog, kodi_utils.sleep, kodi_utils.container_refresh
...
david_str, pos_str, top_pos_str, top_str = ls(32036), ls(32707), ls(32708), ls(32709)
get_icon = 'special://home/addons/plugin.video.david/resources/artwork/%s'
...
class MenuEditor:
	def __init__(self, params):
		...
		else: self.menu_item = dict(parse_qsl(get_infolabel("ListItem.FileNameAndPath").replace('plugin://plugin.video.david/?','')))
	...
	def _menu_select(self, choice_items, menu_name, heading=david_str, multi_line='false', position_list=False):
		def _builder():
			for item in choice_items:
				item_get = item.get
				line2 = pos_str % (menu_name, ls(item_get('name', None) or item_get('list_name')) if position_list else '')
				try: icon = item_get('iconImage', 'discover.png') if item_get('network_id', None) else tp(get_icon % item_get('iconImage'))
				except: icon = tp(get_icon % 'discover.png')
				yield {'line1': ls(item_get('name')), 'line2': line2, 'icon':icon}
		list_items = list(_builder())
# 		if position_list: list_items.insert(0, {'line1': top_str, 'line2': top_pos_str % menu_name, 'icon': get_icon('discover')})
		if position_list: list_items.insert(0, {'line1': top_str, 'line2': top_pos_str % menu_name, 'icon': tp(get_icon % 'discover.png')})
...
# 				yield {'line1': ls(item_get('name')), 'line2': line2, 'icon':icon}
# 		list_items = list(_builder())
# 		if position_list: list_items.insert(0, {'line1': top_str, 'line2': top_pos_str % menu_name, 'icon': tp(get_icon % 'top.png')})
# 		index_list = [list_items.index(i) for i in list_items]
# 		kwargs = {'items': json.dumps(list_items), 'heading': heading, 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': multi_line}
# 		return kodi_utils.select_dialog(index_list, **kwargs)
	...
	def _icon_select(self, folder_first=True):
		...
		kwargs = {'items': json.dumps(list_items), 'heading': david_str, 'window_xml': 'select.xml'}
	...
	def _get_external_name_input(self, current_name):
		new_name = dialog.input(david_str, defaultt=current_name)
	...
	def shortcut_folder_make(self):
		list_name = dialog.input(david_str)
