...
from modules.kodi_utils import sleep, Thread, activeCount, item_jump, container_update, select_dialog, json, get_setting, local_string as ls
...
def make_thread_list(_target, _list, _max_threads=500):
	for item in _list:
...
def make_thread_list_enumerate(_target, _list, _max_threads=500):
...
def sort_list(sort_key, sort_direction, list_data, ignore_articles):
	try:
		...
		elif sort_key == 'random': return sorted(list_data, key=lambda k: random())
