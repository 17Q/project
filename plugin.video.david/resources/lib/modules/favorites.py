...
from modules.kodi_utils import json, notification, container_refresh, select_dialog, confirm_dialog, database, favorites_db, local_string as ls
...
class Favorites:
	...
	def add_to_favorites(self):
		try:
			self.dbcur.execute("INSERT INTO favorites VALUES (?, ?, ?)", (self.media_type, str(self.tmdb_id), self.title))
			notification(32576, 3500)
		except: notification(32574, 3500)

	def remove_from_favorites(self):
		try:
			self.dbcur.execute("DELETE FROM favorites where db_type=? and tmdb_id=?", (self.media_type, str(self.tmdb_id)))
			container_refresh()
			notification(32576, 3500)
		except: notification(32574, 3500)

	def get_favorites(self, media_type):
		self.dbcur.execute('''SELECT tmdb_id, title FROM favorites WHERE db_type=?''', (media_type,))
	...
	def clear_favorites(self):
		...
		self.dbcur.execute("DELETE FROM favorites WHERE db_type=?", (self.media_type,))
...
def retrieve_favorites(media_type, page_no):
