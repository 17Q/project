...
from modules.kodi_utils import json, notification, set_property, make_listitem, list_dirs, delete_file, show_busy_dialog, hide_busy_dialog, get_icon, Thread
...
class Images():
	def run(self, params):
		...
		if len(self.list_items) == 0:
			hide_busy_dialog()
			return notification(32575)
		if not 'in_progress' in params: self.open_window_xml()
		else:
			hide_busy_dialog()
			return self.list_items, self.next_page_params
	...
	def people_image_results(self):
		...
		actor_imdb_id = self.params['actor_imdb_id'] or people_get_imdb_id(actor_name)
		...
		if page_no == 1:
			...
			tmdb_images = [(tmdb_image_base % ('original', i['file_path']), '%s_%sx%s_%03d' % (actor_name, i['height'], i['width'], count),
	...
	def browser_image(self, folder_path, return_items=False):
		def builder():
			for item in files:
				try:
					...
					try: thumb_url = os.path.join(thumbs_path, [i for i in thumbs if i == item][0])
					except: thumb_url = os.path.join(folder_path, item)
					listitem.setProperty('thumb', thumb_url)
					listitem.setProperty('path', image_url)
		...
		thumbs.sort()
		all_images = [(os.path.join(folder_path, i), i)for i in files]
	...
	def delete_image(self):
		...
		set_property('david.delete_image_finished', 'true')
		hide_busy_dialog()
