...
def browse_rd_cloud(folder_id):
	...
	file_info = [dict(i, **{'url_link': torrent_files['links'][idx]}) for idx, i in enumerate([i for i in torrent_files['files'] if i['path'].lower().endswith(tuple(extensions))])]
...
def rd_external_browser(magnet, filtering_list):
	try:
		episode_match, torrent_id, torrent_keys = False, None, []
		...
		for i in files_tuple:
			if any(x in i[1] for x in filtering_list): episode_match = True
			append(i[0])
