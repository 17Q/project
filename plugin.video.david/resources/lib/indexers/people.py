...
notification, get_icon, unquote, container_refresh, ls = kodi_utils.notification, kodi_utils.get_icon, kodi_utils.unquote, kodi_utils.container_refresh, kodi_utils.local_string
...
def person_data_dialog(params):
	...
	if params.get('refresh', False): container_refresh()
...
def person_search(query=None, window_xml='select.xml'):
	...
	if len(people) == 1:
		...
		kwargs = {'items': json.dumps(list_items), 'heading': ls(32036), 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'false', 'window_xml': window_xml}
	...
	return person_data_dialog({'actor_name': actor_name, 'actor_image': actor_image, 'actor_id': actor_id, 'refresh': True})
