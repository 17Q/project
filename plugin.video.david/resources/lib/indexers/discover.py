...
base_str, window_id = '[B]%s:[/B]  [I]%s[/I]', 'DAVID_%s_discover_params'
...
class Discover:
	def __init__(self, params):
		...
		if self.media_type: self.window_id = window_id % self.media_type.upper()
		else: self.window_id = ''
		try: self.discover_params = json.loads(get_property(self.window_id))
	...
	def recommended(self):
		...
		title = dialog.input(heading_base % ls(32228))
		if not title: return
		if self.media_type == 'movie': function = tmdb_api.tmdb_movies_title_year
	...
	def year(self):
		if self._action(self.key) in ('clear', None): return
		if self.discover_params['media_type'] == 'movie': years = meta_lists.years_movies
		else: years = meta_lists.years_tvshows
	...
	def history(self, media_type=None, display=True):
		def _builder():
			for count, item in enumerate(data):
				try:
					...
					name = item['name']
					url_params = {'mode': item['mode'], 'action': item['action'], 'query': item['query'], 'name': name, 'iconImage': default_icon}
		...
		string = 'david_discover_%s_%%' % media_type
	...
	def help(self):
		text_file = translate_path('special://home/addons/plugin.video.david/resources/text/tips/135. Discover.txt')
	...
	def _clear_property(self):
		clear_property(self.window_id)
		self.discover_params = {}
		container_refresh()

	def _set_property(self):
		return set_property(self.window_id, json.dumps(self.discover_params))
	...
	def _build_string(self):
		...
		if 'with_genres' in string_params: string += string_params['with_genres']
		if 'without_genres' in string_params: string += string_params['without_genres']
		if 'with_keywords' in string_params: string += string_params['with_keywords']
		if 'without_keywords' in string_params: string += string_params['without_keywords']
...
def set_history(media_type, name, query):
	string = 'david_discover_%s_%s' % (media_type, query)
	cache = main_cache.get(string)
	if cache: return
	if media_type == 'movie': mode, action = 'build_movie_list', 'tmdb_movies_discover'
	else: mode, action = 'build_tvshow_list', 'tmdb_tv_discover'
	data = {'mode': mode, 'action': action, 'name': name, 'query': query}
	main_cache.set(string, data, expiration=timedelta(days=365))
	return
...
def remove_from_history(params):
	...
	container_refresh()
	if not params['silent']: notification(32576)

def remove_all_history(params):
	media_type = params['media_type']
	if not confirm_dialog(): return
	all_history = Discover({}).history(media_type, display=False)
	for item in all_history: remove_from_history({'data_id': item, 'silent': True})
	notification(32576)
