...
from modules.kodi_utils import json, translate_path, hide_busy_dialog, dialog, select_dialog, ok_dialog, addon_fanart, empty_poster, local_string as ls
from modules.settings import get_art_provider, provider_sort_ranks, get_fanart_data
# from modules.kodi_utils import logger

media_folder = 'special://home/addons/plugin.video.david/resources/skins/Default/media/%s'
...
class SourceResults(BaseDialog):
	...
	def get_provider_and_path(self, provider):
		...
		else: provider, provider_path = 'folders', translate_path('special://home/addons/plugin.video.david/resources/skins/Default/media/providers/folders.png')
	...
	def onAction(self, action):
		...
		if action == self.info_action:
			...
			choice = self.open_window(('windows.sources', 'ResultsContextMenu'), 'contextmenu.xml',
									item=source, meta=self.meta, filter_applied=self.filter_applied)
			if choice:
					...
					if scrape_provider == 'external':
						...
						if 'cache_provider' in item:
							if 'Uncached' in item['cache_provider']:
								...
								if pack:
									set_property('source_type', 'CACHED [B]PACK[/B]')
								else:
									set_property('source_type', 'CACHED')
	...
	def set_properties(self):
		...
		self.setProperty('clearlogo', self.meta.get(self.clearlogo_main) or self.meta.get(self.clearlogo_backup) or '')
		self.setProperty('title', self.meta['title'])
		self.setProperty('plot', self.meta['plot'])
	...
	def filter_results(self):
		...
		if main_choice in ('quality', 'provider'):
			...
			filtered_list = self.make_items(self.uncached_torrents)
...
class ResultsInfo(BaseDialog):
	...
	def get_provider_and_path(self):
		provider = lower(self.item.getProperty('provider'))
		if provider in info_icons_dict: provider_path = info_icons_dict[provider]
		else: provider_path = translate_path('special://home/addons/plugin.video.david/resources/skins/Default/media/providers/folders.png')
		return provider, provider_path

	def get_quality_and_path(self):
		quality = lower(self.item.getProperty('quality'))
		quality_path = info_quality_dict[quality]
		return quality, quality_path
	...
	def set_properties(self):
		...
		self.setProperty('results.info.name', self.item.getProperty('name'))
		self.setProperty('results.info.source_type', self.item.getProperty('source_type'))
		self.setProperty('results.info.source_site', self.item.getProperty('source_site'))
		self.setProperty('results.info.size_label', self.item.getProperty('size_label'))
		self.setProperty('results.info.extra_info', self.item.getProperty('extra_info'))
		self.setProperty('results.info.highlight', self.item.getProperty('highlight'))
		self.setProperty('results.info.hash', self.item.getProperty('hash'))
		self.setProperty('results.info.provider', provider)
		self.setProperty('results.info.quality', quality)
		self.setProperty('results.info.provider_icon', provider_path)
		self.setProperty('results.info.quality_icon', quality_path)
...
class ResultsContextMenu(BaseDialog):
	...
	def onAction(self, action):
		if action in self.selection_actions:
			chosen_listitem = self.get_listitem(self.window_id)
			self.selected = chosen_listitem.getProperty('context.action')
			return self.close()
...
class SourceResultsChooser(BaseDialog):
	...
	def onAction(self, action):
		...
		if action in self.selection_actions:
			chosen_listitem = self.get_listitem(self.window_id)
			self.choice = chosen_listitem.getProperty('window.name')
			self.close()
	...
	def make_items(self):
		...
		for item in self.xml_choices:
			listitem = self.make_listitem()
			listitem.setProperty('window.name', item[0])
			listitem.setProperty('window.image', item[1])
			append(listitem)
