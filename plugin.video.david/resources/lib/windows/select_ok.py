	...
	def make_menu(self):
		def builder():
			for count, item in enumerate(self.items, 1):
				...
				if 'icon' in item: listitem.setProperty('dialog.icon', item['icon'])
				else: listitem.setProperty('dialog.default_icon', 'true')
				listitem.setProperty('dialog.line1', line1)
				listitem.setProperty('dialog.line2', line2)
				yield listitem
		enum = self.enumerate == 'true'
		self.item_list = list(builder())

	def set_properties(self):
		self.setProperty('dialog.multi_choice', self.multi_choice)
		self.setProperty('dialog.multi_line', self.multi_line)
		self.setProperty('dialog.heading', self.heading)
...
class Confirm(BaseDialog):
	...
	def set_properties(self):
		self.setProperty('dialog.ok_label', self.ok_label)
		self.setProperty('dialog.cancel_label', self.cancel_label)
		self.setProperty('dialog.text', self.text)
		self.setProperty('dialog.heading', self.heading)
...
class SelectContextMenu(BaseDialog):
	...
	def make_menu(self):
		if self.context_menu_type == 'imdb_keywords':
			...
			keyword = self.list_item.getProperty('dialog.line1')
