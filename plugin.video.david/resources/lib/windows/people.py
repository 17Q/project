...
class People(BaseDialog):
	...
	def onInit(self):
		Thread(target=self.make_trivia).start()
		Thread(target=self.make_videos).start()
	...
	def onAction(self, action):
		...
		if action in self.selection_actions:
			...
			if self.control_id in (more_from_movies_id, more_from_tvshows_id, more_from_director_id):
				...
				thumb = chosen_listitem.getProperty('person.thumbnail')
				chosen = dialogs.imdb_videos_choice(getattr(self, chosen_var)[self.get_position(self.control_id)]['videos'], thumb)
	...
	def make_more_from(self, media_type):
		try:
			...
			self.setProperty('person.more_from_%s.number' % media_type, '(x%02d)' % len(item_list))
			self.item_action_dict[_id] = 'person.tmdb_id'
			self.add_items(_id, item_list)
		except: pass

	def make_videos(self):
		def builder():
			for count, item in enumerate(self.all_videos, 1):
				try:
					listitem = self.make_listitem()
					listitem.setProperty('person.name', '%01d. %s' % (count, item['title']))
					listitem.setProperty('person.thumbnail', item['poster'])
					listitem.setProperty('person.content_list', 'all_videos')
					yield listitem
				except: pass
		try:
			self.all_videos = imdb_videos(self.imdb_id)
			item_list = list(builder())
			self.setProperty('person.imdb_videos.number', '(x%02d)' % len(item_list))
			self.item_action_dict[videos_id] = 'person.content_list'
	...
	def make_trivia(self):
		...
		def builder():
			for item in self.all_trivia:
				try:
					listitem = self.make_listitem()
					listitem.setProperty('person.text', item)
					listitem.setProperty('person.content_list', 'all_trivia')
					yield listitem
				except: pass
		try:
			self.all_trivia = imdb_people_trivia(self.person_imdb_id)
			item_list = list(builder())
			self.setProperty('person.imdb_trivia.number', '(x%02d)' % len(item_list))
			self.item_action_dict[trivia_id] = 'person.content_list'
	...
	def make_tmdb_listitems(self, data, media_type):
		...
		for item in data:
			try:
				...
				listitem.setProperty('person.name', item[name_key])
				listitem.setProperty('person.release_date', year)
				listitem.setProperty('person.vote_average', '%.1f' % item['vote_average'])
				listitem.setProperty('person.thumbnail', thumbnail)
				listitem.setProperty('person.tmdb_id', str(tmdb_id))
				append(tmdb_id)
				yield listitem
			except: pass
	...
	def show_text_media_list(self, chosen_var):
		return self.open_window(('windows.extras', 'ShowTextMedia'), 'textviewer_media_list.xml',
								items=getattr(self, chosen_var), current_index=self.get_position(self.control_id), poster=self.person_image)
	...
	def set_properties(self):
		self.setProperty('person.name', self.person_name)
		self.setProperty('person.id', str(self.person_id))
		self.setProperty('person.image', self.person_image)
		self.setProperty('person.fanart', addon_fanart)
		self.setProperty('person.gender', self.person_gender)
		self.setProperty('person.place_of_birth', self.person_place_of_birth)
		self.setProperty('person.biography', self.person_biography)
		self.setProperty('person.birthday', self.person_birthday)
		self.setProperty('person.deathday', self.person_deathday)
		self.setProperty('person.age', str(self.person_age))
		self.setProperty('person.enable_scrollbars', self.enable_scrollbars)
		self.setProperty('person.enable_animation', self.enable_animation)