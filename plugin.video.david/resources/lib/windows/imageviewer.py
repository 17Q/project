...
class ThumbImageViewer(BaseDialog):
	...
	def make_next_page(self):
		try:
			...
			listitem.setProperty('name', ls(32799))
	...
	def set_properties(self):
		...
		self.setProperty('thumbviewer.fanart', addon_fanart)
	...
	def make_items(self):
		def builder():
			for item in self.all_images:
				try:
					listitem = self.make_listitem()
					listitem.setProperty('slideshow.image', item[0])
					listitem.setProperty('slideshow.title', item[1])
					yield listitem
				except: pass
		self.item_list = list(builder())

	def set_properties(self):
		self.setProperty('slideshow.fanart', addon_fanart)
