...
movie_data, tvshow_data, default_fanarttv_data, english_translation_function = tmdb.movie_details, tmdb.tvshow_details, fanarttv.default_fanart_nometa, tmdb.english_translation
...
metacache_get, metacache_set, fanarttv_get, fanarttv_add, jsondate_to_datetime_function = metacache.get, metacache.set, fanarttv.get, fanarttv.add, jsondate_to_datetime
...
def movie_meta(id_type, media_id, user_info, current_date):
	...
	try:
		...
		try:
			poster = tmdb_image_url % (image_resolution['poster'], images_get('posters')[0]['file_path'])
		...
		try:
			clearlogo = tmdb_image_url % (image_resolution['clearlogo'], images_get('logos')[0]['file_path'])
		...
		if fanarttv_data: meta.update(fanarttv_data)
		else: meta.update(default_fanarttv_data)
		metacache_set('movie', id_type, meta, movie_expiry(current_date, meta))
	except: pass
	return meta
