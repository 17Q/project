...
class MetaCache:
	def get(self, media_type, id_type, media_id):
		...
		try:
			...
			if meta is None:
				...
				if media_type == 'season': cache_data = dbcur.execute(GET_SEASON, (media_id,)).fetchone()
				else: cache_data = dbcur.execute(GET_MOVIE_SHOW % id_type, (media_type, media_id)).fetchone()
				if cache_data:
	...
	def get_memory_cache(self, media_type, id_type, media_id, current_time):
		result = None
		try:
			if media_type == 'season': prop_string = 'meta_season_%s' % media_id
			else: prop_string = '%s_%s_%s' % (media_type, id_type, media_id)
	...
	def set_memory_cache(self, media_type, id_type, meta, expires, media_id):
		try:
			media_id = string(media_id)
			if media_type == 'season': cachedata, prop_string = (expires, meta), 'meta_season_%s' % string(media_id)
			else: cachedata, prop_string = (expires, meta), '%s_%s_%s' % (media_type, id_type, string(media_id))
			set_property(prop_string, repr(cachedata))
		except: pass

	def delete_memory_cache(self, media_type, id_type, media_id):
		try:
			if media_type == 'season': clear_property('meta_season_%s' % media_id)
			else: clear_property('%s_%s_%s' % (media_type, id_type, media_id))
		except: pass
