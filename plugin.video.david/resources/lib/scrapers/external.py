...
confirm_progress_media = kodi_utils.confirm_progress_media
...
class source:
	def __init__(self, source_dict, debrid_torrents, debrid_hosters, internal_scrapers, prescrape_sources, progress_dialog, disabled_ext_ignored=False):
		...
		self.timeout = 60 if disabled_ext_ignored else int(get_setting('scrapers.timeout.1', '30'))
			...
			while not self.progress_dialog.iscanceled():
				try:
					...
					if self.threads_completed:
						...
						elif percent >= 100: close_dialog = False; break
