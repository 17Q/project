...
icon = kodi_utils.translate_path('special://home/addons/plugin.video.david/resources/artwork/realdebrid.png')
...
class RealDebridAPI:
	...
	def auth(self):
		...
		content = line % (ls(32517), ls(32700) % 'https://real-debrid.com/device', ls(32701) % user_code)
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('david_rd_user_cloud',))
				try: 
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_rd_user_cloud',))
					clear_property("david_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_rd_user_cloud_info_%s' % i,))
						clear_property("david_rd_user_cloud_info_%s" % i)
					dbcon.commit()
					user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_rd_downloads',))
				clear_property("david_rd_downloads")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_rd_valid_hosts',))
				clear_property('david_rd_valid_hosts')
				dbcon.commit()
				dbcon.close()
