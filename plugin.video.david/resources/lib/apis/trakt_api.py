...
from modules.utils import sort_list, sort_for_article, make_thread_list, paginate_list, jsondate_to_datetime as js2date
...
requests, execute_builtin, select_dialog, container_refresh = kodi_utils.requests, kodi_utils.execute_builtin, kodi_utils.select_dialog, kodi_utils.container_refresh
# trakt_icon = kodi_utils.get_icon('trakt')
trakt_icon = kodi_utils.translate_path('special://home/addons/plugin.video.david/resources/artwork/trakt.png')
trakt_str = ls(32037)
API_ENDPOINT = 'https://api.trakt.tv/%s'
timeout = 40
session = kodi_utils.make_session('https://api.trakt.tv')
...
def trakt_get_device_token(device_codes):
	...
	try:
		...
		content = '[CR]%s[CR]%s' % (ls(32700) % str(device_codes['verification_url']), ls(32701) % str(device_codes['user_code']))
		progressDialog = kodi_utils.progress_dialog('%s %s' % (ls(32037), ls(32057)), trakt_icon)
...
def trakt_collection(media_type, page_no):
	...
	lists_sort_order = settings.lists_sort_order('collection')
	if lists_sort_order == 0: original_list = sort_for_article(original_list, 'title', settings.ignore_articles())
	elif lists_sort_order == 1: original_list.sort(key=lambda k: k['collected_at'], reverse=True)
	else: original_list.sort(key=lambda k: k['released'], reverse=True)
	if settings.paginate(): final_list, total_pages = paginate_list(original_list, page_no, settings.page_limit())
...
def trakt_watchlist(media_type, page_no):
	...
	lists_sort_order = settings.lists_sort_order('watchlist')
	if lists_sort_order == 0: original_list = sort_for_article(original_list, 'title', settings.ignore_articles())
	elif lists_sort_order == 1: original_list.sort(key=lambda k: k['collected_at'], reverse=True)
	else: original_list.sort(key=lambda k: k['released'], reverse=True)
	if settings.paginate(): final_list, total_pages = paginate_list(original_list, page_no, settings.page_limit())
...
def make_new_trakt_list(params):
	mode = params['mode']
	list_title = kodi_utils.dialog.input(ls(32036))
...
def trakt_sync_activities(force_update=False):
	...
	trakt_cache.clear_trakt_list_contents_data('user_lists')
