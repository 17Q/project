...
user_agent = 'david_for_kodi'
timeout = 20.0
requests = kodi_utils.make_requests()
# icon = kodi_utils.get_icon('alldebrid')
icon = kodi_utils.translate_path('special://home/addons/plugin.video.david/resources/artwork/alldebrid.png')
...
class AllDebridAPI:
	...
	def auth(self):
		...
		sleep_interval = 5
		content = line % (ls(32517), ls(32700) % response.get('base_url'), ls(32701) % response.get('pin'))
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_ad_user_cloud',))
				clear_property('david_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_ad_valid_hosts',))
				clear_property('david_ad_valid_hosts')
				dbcon.commit()
				dbcon.close()
