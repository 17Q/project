...
def tmdb_movies_title_year(title, year=None):
	if year:
		...
		url = '%s/search/movie?api_key=%s&language=en-US&include_adult=true&query=%s&year=%s' % (base_url, tmdb_api_key(), title, year)
	else:
		string = 'tmdb_movies_title_year_%s' % title
		url = '%s/search/movie?api_key=%s&language=en-US&include_adult=true&query=%s' % (base_url, tmdb_api_key(), title)
...
def tmdb_tv_title_year(title, year=None):
	if year:
		...
		url = '%s/search/tv?api_key=%s&language=en-US&include_adult=true&query=%s&first_air_date_year=%s' % (base_url, tmdb_api_key(), title, year)
	else:
		string = 'tmdb_tv_title_year_%s' % title
		url = '%s/search/tv?api_key=%s&language=en-US&query=%s' % (base_url, tmdb_api_key(), title)
	return cache_object(get_tmdb, string, url, expiration=EXPIRY_1_MONTH)

def tmdb_tv_popular(page_no):
	string = 'tmdb_tv_popular_%s' % page_no
	url = '%s/tv/popular?api_key=%s&language=en-US&include_adult=true&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key(), page_no)
	return cache_object(get_tmdb, string, url, expiration=EXPIRY_2_DAYS)
...
def tmdb_people_info(query):
	...
	url = '%s/search/person?api_key=%s&language=en-US&include_adult=true&query=%s' % (base_url, tmdb_api_key(), query)
