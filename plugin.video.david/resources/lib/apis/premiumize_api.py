...
icon = kodi_utils.translate_path('special://home/addons/plugin.video.david/resources/artwork/premiumize.png')
...
class PremiumizeAPI:
	...
	def auth(self):
		...
		content = line % (ls(32517), ls(32700) % response.get('verification_uri'), ls(32701) % response.get('user_code'))
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('david_pm_user_cloud%',))
				try:
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_pm_transfers_list',))
				clear_property("david_pm_transfers_list")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('david_pm_valid_hosts',))
				clear_property('david_pm_valid_hosts')
				dbcon.commit()
				dbcon.close()
