...
from patriotscrapers.modules import cleandate
...
from patriotscrapers.modules import utils
...
V2_API_KEY = control.addon('plugin.video.patriot').getSetting('trakt.client_id')
CLIENT_SECRET = control.addon('plugin.video.patriot').getSetting('trakt.client_secret')
    ...
    try:
        if getTraktCredentialsInfo() == True:
            if control.yesnoDialog(control.lang(32511) + '[CR]' + control.lang(32512), heading='Trakt'):
                control.addon('plugin.video.patriot').setSetting(id='trakt.user', value='')
                control.addon('plugin.video.patriot').setSetting(id='trakt.authed', value='')
                control.addon('plugin.video.patriot').setSetting(id='trakt.token', value='')
                control.addon('plugin.video.patriot').setSetting(id='trakt.refresh', value='')
            raise Exception()
        ...
        print('info - ' + token)
        control.addon('plugin.video.patriot').setSetting(id='trakt.user', value=user)
        control.addon('plugin.video.patriot').setSetting(id='trakt.authed', value=authed)
        control.addon('plugin.video.patriot').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.patriot').setSetting(id='trakt.refresh', value=refresh)
        raise Exception()
    except:
...
def getTraktCredentialsInfo():
    user = control.addon('plugin.video.patriot').getSetting('trakt.user').strip()
    token = control.addon('plugin.video.patriot').getSetting('trakt.token')
    refresh = control.addon('plugin.video.patriot').getSetting('trakt.refresh')
    if (user == '' or token == '' or refresh == ''): return False
    return True


def getTraktIndicatorsInfo():
    indicators = control.addon('plugin.video.patriot').getSetting('indicators') if getTraktCredentialsInfo() == False else control.addon('plugin.video.patriot').getSetting('indicators.alt')
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.addon('plugin.video.patriot').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncMovies():
    timeout = cache.timeout(syncMovies, control.addon('plugin.video.patriot').getSetting('trakt.user').strip())
    return timeout
...
def cachesyncTVShows(timeout=0):
    indicators = cache.get(syncTVShows, timeout, control.addon('plugin.video.patriot').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncTVShows():
    timeout = cache.timeout(syncTVShows, control.addon('plugin.video.patriot').getSetting('trakt.user').strip())
