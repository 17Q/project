...
DEBUGPREFIX = '[ PatriotScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ PatriotScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'patriot.log')
debug_enabled = control.addon('plugin.video.patriot').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('PatriotScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
