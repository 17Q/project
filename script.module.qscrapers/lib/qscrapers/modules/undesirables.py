...
class Undesirables():
	...
	def set_defaults(self):
		from qscrapers.modules.source_utils import UNDESIRABLES
...
def undesirablesSelect():
	from qscrapers.modules.control import multiselectDialog
	from qscrapers.modules.source_utils import UNDESIRABLES
...
def undesirablesUserRemove():
	from qscrapers.modules.control import multiselectDialog, notification
	undesirables_cache = Undesirables()
...
def undesirablesUserRemoveAll():
	from qscrapers.modules.control import yesnoDialog, notification
	undesirables_cache = Undesirables()
	user_undesirables = undesirables_cache.get_user_defined()
	if not user_undesirables: return notification(message=32084)
...
def undesirablesInput():
	from qscrapers.modules.control import dialog
	undesirables_cache = Undesirables()
