# created by Venom for Qscrapers (updated 3-02-2022) modified by kodifitzwell
"""
	Qscrapers Project
"""
...
# Currently supports BITSEARCH(+), EZTV(+), ThePirateBay(+), TheRARBG(+), YTS(+)
	...
	def sources(self, data, hostDict):
		...
		sources = []
		if not data:
			homeWindow.clearProperty('qscrapers.comet.performing_single_scrape')
			return sources
		...
		try:
			...
			if 'tvshowtitle' in data:
				homeWindow.setProperty('qscrapers.comet.performing_single_scrape', 'true')
				season = data['season']
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		while count < 10000 and not finished_single_scrape:
			finished_single_scrape = homeWindow.getProperty('qscrapers.comet.performing_single_scrape') != 'true'
			sleep(100)
			count += 100
