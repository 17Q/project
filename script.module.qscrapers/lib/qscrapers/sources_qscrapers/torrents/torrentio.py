# -*- coding: utf-8 -*-
# created by Venom for Qscrapers (updated 7-19-2022) ud (updated 05/22/24)
'''
	Qscrapers Project
'''
...
from qscrapers.modules import source_utils, cache
from qscrapers.modules import log_utils
from time import time
from qscrapers.modules.control import setting as getSetting, homeWindow, sleep
...
# Currently supports YTS(+), EZTV(+), RARBG(+), 1337x(+), ThePirateBay(+), KickassTorrents(+), TorrentGalaxy(+), HorribleSubs(+), NyaaSi(+), NyaaPantsu(+), Rutor(+), Comando(+), ComoEuBaixo(+), Lapumia(+), OndeBaixa(+), Torrent9(+).
	...
	def sources(self, data, hostDict):
		...
		sources = []
		if not data:
			homeWindow.clearProperty('qscrapers.torrentio.performing_single_scrape')
			return sources
		...
		try:
			...
			if 'tvshowtitle' in data:
				homeWindow.setProperty('qscrapers.torrentio.performing_single_scrape', 'true')
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		while count < 10000 and not finished_single_scrape:
			finished_single_scrape = homeWindow.getProperty('qscrapers.torrentio.performing_single_scrape') != 'true'
			sleep(100)
			count += 100
