...
def language_invoker_choice(params):
	...
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.htmc247/addon.xml')
...
def addon_icon_choice(params):
	...
	icons_path = 'special://home/addons/plugin.video.htmc247/resources/media/addon_icons'
...
def personallists_manager_choice(params):
	...
	list_type = params['list_type']
	all_lists = get_all_personal_lists(get_setting('htmc247.personal_list.list_sort', '0'))
	choices = []
...
def set_quality_choice(params):
	...
	# Edit for 247
	dl = ['Include 1080p', 'Include 720p', 'Include SD']
	fl = ['1080p', '720p', 'SD']
	q_setting = get_setting('htmc247.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('htmc247.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('htmc247.', ''), default_setting_values(setting_id.replace('htmc247.', ''))['setting_default'])
					button_action = get_setting('htmc247.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('htmc247.%s' % filter_setting_id).split(', ')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or kodi_utils.get_icon('htmc247')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('htmc247.%s' % setting), default_setting_values(setting)['setting_default']
...
def options_menu_choice(params, meta=None):
	...
	single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_htmc247', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
