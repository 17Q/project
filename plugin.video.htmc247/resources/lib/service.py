...
pause_services_prop = 'htmc247.pause_services'
firstrun_update_prop = 'htmc247.firstrun_update'
current_skin_prop = 'htmc247.current_skin'
...
class SetAddonConstants:
	def run(self):
		kodi_utils.logger('Htmc 247', 'SetAddonConstants Service Starting')
		_info = kodi_utils.addon().getAddonInfo
		addon_items = [('htmc247.addon_version', _info('version')),
					('htmc247.addon_path', _info('path')),
					('htmc247.addon_profile', kodi_utils.translate_path(_info('profile'))),
					('htmc247.addon_icon', kodi_utils.translate_path(_info('icon'))),
					('htmc247.addon_fanart', kodi_utils.translate_path(_info('fanart')))]
		for item in addon_items: kodi_utils.set_property(*item)
		return kodi_utils.logger('Htmc 247', 'SetAddonConstants Service Finished')

class DatabaseMaintenance:
	def run(self):
		kodi_utils.logger('Htmc 247', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import make_databases
		make_databases()
		return kodi_utils.logger('Htmc 247', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		kodi_utils.logger('Htmc 247', 'SyncSettings Service Starting')
		sync_settings()
		return kodi_utils.logger('Htmc 247', 'SyncSettings Service Finished')

class OnUpdateChanges:
	def run(self):
		kodi_utils.logger('Htmc 247', 'OnUpdateChanges Service Starting')
		try:
			for method in list(filter(lambda x: x[0] != 'run', inspect.getmembers(OnUpdateChanges, predicate=inspect.isfunction))):
				if not get_setting('htmc247.updatechecks.%s' % method[0], 'false') == 'true':
					method[1](self)
					set_setting('updatechecks.%s' % method[0], 'true')
		except: pass
		return kodi_utils.logger('Htmc 247', 'OnUpdateChanges Service Finished')
	...
	def personal_lists_01(self):
		...
		try:
			...
			try:
				...
				dbcon.close()
		except Exception as e: kodi_utils.logger('Htmc 247', 'OnUpdateChanges Service (personal_lists_01) Changes Failed: %s' % str(e))
	...
	def personal_lists_02(self):
		...
		except Exception as e: kodi_utils.logger('Htmc 247', 'OnUpdateChanges Service (personal_lists_02) Changes Failed: %s' % str(e))

class CustomFonts:
	def run(self):
		kodi_utils.logger('Htmc 247', 'CustomFonts Service Starting')
		...
		return kodi_utils.logger('Htmc 247', 'CustomFonts Service Finished')

class TraktMonitor:
	def run(self):
		kodi_utils.logger('Htmc 247', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('Htmc 247', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else:
					if status in ('success', 'no account'): kodi_utils.logger('Htmc 247', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else: kodi_utils.logger('Htmc 247', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('htmc247.trakt.refresh_widgets', 'false') == 'true': kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('Htmc 247', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return kodi_utils.logger('Htmc 247', 'TraktMonitor Service Finished')

class UpdateCheck:
	def run(self):
		if kodi_utils.get_property(firstrun_update_prop) == 'true': return
		kodi_utils.logger('Htmc 247', 'UpdateCheck Service Starting')
		...
		return kodi_utils.logger('Htmc 247', 'UpdateCheck Service Finished')

class WidgetRefresher:
	def run(self):
		kodi_utils.logger('Htmc 247', 'WidgetRefresher Service Starting')
		from time import time
		...
		while not monitor.abortRequested():
			try:
				wait_for_abort(10)
				offset = int(get_setting('htmc247.widget_refresh_timer', '60'))
				if offset != self.offset:
					self.set_next_refresh(time())
					continue
				if self.condition_check(): continue
				if self.next_refresh < time():
					kodi_utils.logger('Htmc 247', 'WidgetRefresher Service - Widgets Refreshed')
		...
		return kodi_utils.logger('Htmc 247', 'WidgetRefresher Service Finished')
	...
	def condition_check(self):
		...
		if kodi_utils.get_property('htmc247.window_loaded') == 'true': return True 
		try:
			window_stack = json.loads(kodi_utils.get_property('htmc247.window_stack'))
			if window_stack or window_stack == []: return True
		except: pass
		return False

	def set_next_refresh(self, _time):
		self.offset = int(get_setting('htmc247.widget_refresh_timer', '60'))
		if self.offset: self.next_refresh = _time + (self.offset*60)
		else: self.next_refresh = None

class AutoStart:
	def run(self):
		kodi_utils.logger('Htmc 247', 'AutoStart Service Starting')
		from modules.settings import auto_start_htmc247
		if auto_start_htmc247(): kodi_utils.run_addon()
		return kodi_utils.logger('Htmc 247', 'AutoStart Service Finished')

class AddonXMLCheck:
	def run(self):
		kodi_utils.logger('Htmc 247', 'AddonXMLCheck Service Starting')
		from xml.dom.minidom import parse as mdParse
		self.addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.htmc247/addon.xml')
		...
		return kodi_utils.logger('Htmc 247', 'AddonXMLCheck Service Finished')

	def check_property(self, setting, tag_name):
		current_addon_setting = get_setting('htmc247.%s' % setting, None)
...
class Htmc247Monitor(Monitor):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			kodi_utils.logger('OnNotificationActions', 'PAUSING Htmc 247 Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			kodi_utils.clear_property(pause_services_prop)
			kodi_utils.logger('OnNotificationActions', 'UNPAUSING Htmc 247 Services Due to Device Awake')

kodi_utils.logger('Htmc 247', 'Main Monitor Service Starting')
Htmc247Monitor().waitForAbort()
kodi_utils.logger('Htmc 247', 'Main Monitor Service Finished')
