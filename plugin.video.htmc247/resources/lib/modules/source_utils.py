...
def internal_results(provider, sources):
	set_property('htmc247.internal_results.%s' % provider, json.dumps(sources))
...
def get_external_cache_status(data, debrid, token, services, unchecked_hashes):
	...
	def _busy_check(service):
		...
		prop = 'htmc247.%s_cache_check' % service
	...
	def _process(service, unchecked_hashes):
		...
		if service in ('mediafusion', 'comet', 'torrentio'):
			if service == 'mediafusion':
				_busy_check(service)
				set_property('htmc247.%s_cache_check' % service, 'true')
			...
			if service in ('mediafusion', 'comet'): set_property('htmc247.%s_cache_check' % service, 'false')
		elif service == 'dmm':
			def get_secret():
	...
	try:
		results = []
		token = get_setting('htmc247.%s' % token)
