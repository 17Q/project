...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.htmc247').getAddonInfo(info)

def addon_version():
	return get_property('htmc247.addon_version') or addon_info('version')

def addon_path():
	return get_property('htmc247.addon_path') or addon_info('path')

def addon_profile():
	return get_property('htmc247.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('htmc247.addon_icon') or addon_info('icon')

def addon_fanart():
	return get_property('htmc247.addon_fanart') or addon_info('fanart')

def get_icon(image_name, image_folder='icons'):
	return translate_path('special://home/addons/plugin.video.htmc247/resources/media/%s/%s.png' % (image_folder, image_name))

def get_addon_fanart():
	return get_property('htmc247.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.htmc247/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('htmc247.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('htmc247.%s' % view_type) or None
...
def addon(addon_id='plugin.video.htmc247'):
...
def run_addon(addon='plugin.video.htmc247', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'htmc247' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.htmc247'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.htmc247'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('htmc247.external_scraper.module')
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('Htmc 247', line1, icon or addon_icon(), time)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('htmc247.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('htmc247.playback.volumecheck_percent') or '50'), 100)
...
def get_all_icon_vars():
	icon_items = list_dirs(translate_path('special://home/addons/plugin.video.htmc247/resources/media/icons'))[1]
