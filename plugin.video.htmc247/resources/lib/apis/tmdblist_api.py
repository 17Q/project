...
# 		self.base_url = 'https://api.themoviedb.org/4'
		...
		# self.read_access_token = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyNTNmOGVkNDliMTkzNjk4NDM4MjdkYzkyOGQxMzZlZCIsIm5iZiI6MTc1NjQ2MTk4NS40MDcsInN1YiI6IjY4YjE3YmExNzI5MDliMmY2ODFmMTY3OSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.RisVCZYk49G6EtvEGHAcqqCGKiwdblBNTjRpes5Jke0'
								# 'gzNTQ0MTZhIiwic2NvcGVzIjpbImFwaV9yZWFkIl0sInZlcnNpb24iOjF9.8uevSMakSrdZb1t0ze4OIxq6PoL4N6DZN4VVkKUCayg'
...
class TMDbListAPI:
	...
	# Adds a default list in settings but no lists show in TMDB Lists
	# def revoke(self):
	# 	import requests
	# 	headers = {'accept': 'application/json', 'content-type': 'application/json', 'Authorization': 'Bearer %s' % self.read_access_token}
	# 	data = requests.delete('%s/auth/access_token' % self.base_url, json={'access_token': self.read_access_token}, headers=headers, timeout=20).json()
	...
	# 	else:
	# 		notice = 'Success Auth Revoke'
	# 		set_setting('tmdb.token', '253f8ed49b19369843827dc928d136ed')
	# 		set_setting('tmdb.account_id', '253f8ed49b19369843827dc928d136ed')
	# 		tmdb_lists_cache.clear_all()
	# 	return notification(notice)
	...
	def get_user_lists(self):
		...
		def _process(dummy):
			...
			return results
		account_id = get_setting('htmc247.tmdb.account_id')
	...
	def request_data(self, url, params=None, data=None, method='get'):
		headers = {'accept': 'application/json', 'content-type': 'application/json', 'Authorization': 'Bearer %s' % get_setting('htmc247.tmdb.token')}
