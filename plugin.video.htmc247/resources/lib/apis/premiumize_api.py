...
class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('htmc247.pm.token', 'empty_setting')
	...
	def headers(self):
		return {'User-Agent': 'Htmc 247 for Kodi', 'Authorization': 'Bearer %s' % self.token}

	def _get(self, url, data={}):
		if self.token in ('empty_setting', ''): return None
		headers = {'User-Agent': 'Htmc 247 for Kodi', 'Authorization': 'Bearer %s' % self.token}
