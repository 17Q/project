...
pause_services_prop = 'patriotlight.pause_services'
firstrun_update_prop = 'patriotlight.firstrun_update'
current_skin_prop = 'patriotlight.current_skin'
...
class SetAddonConstants:
	def run(self):
		kodi_utils.logger('Patriot Light', 'SetAddonConstants Service Starting')
		_info = kodi_utils.addon().getAddonInfo
		addon_items = [('patriotlight.addon_version', _info('version')),
					('patriotlight.addon_path', _info('path')),
					('patriotlight.addon_profile', kodi_utils.translate_path(_info('profile'))),
					('patriotlight.addon_icon', kodi_utils.translate_path(_info('icon'))),
					('patriotlight.addon_fanart', kodi_utils.translate_path(_info('fanart')))]
		for item in addon_items: kodi_utils.set_property(*item)
		return kodi_utils.logger('Patriot Light', 'SetAddonConstants Service Finished')

class DatabaseMaintenance:
	def run(self):
		kodi_utils.logger('Patriot Light', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import make_databases
		make_databases()
		return kodi_utils.logger('Patriot Light', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		kodi_utils.logger('Patriot Light', 'SyncSettings Service Starting')
		sync_settings()
		return kodi_utils.logger('Patriot Light', 'SyncSettings Service Finished')

class OnUpdateChanges:
	def run(self):
		kodi_utils.logger('Patriot Light', 'OnUpdateChanges Service Starting')
		try:
			for method in list(filter(lambda x: x[0] != 'run', inspect.getmembers(OnUpdateChanges, predicate=inspect.isfunction))):
				if not get_setting('patriotlight.updatechecks.%s' % method[0], 'false') == 'true':
					method[1](self)
					set_setting('updatechecks.%s' % method[0], 'true')
		except: pass
		return kodi_utils.logger('Patriot Light', 'OnUpdateChanges Service Finished')
	...
	def personal_lists_01(self):
		...
		try:
			...
			try:
				...
				dbcon.close()
		except Exception as e: kodi_utils.logger('Patriot Light', 'OnUpdateChanges Service (personal_lists_01) Changes Failed: %s' % str(e))
	...
	def personal_lists_02(self):
		...
		except Exception as e: kodi_utils.logger('Patriot Light', 'OnUpdateChanges Service (personal_lists_02) Changes Failed: %s' % str(e))

class CustomFonts:
	def run(self):
		kodi_utils.logger('Patriot Light', 'CustomFonts Service Starting')
		...
		return kodi_utils.logger('Patriot Light', 'CustomFonts Service Finished')

class TraktMonitor:
	def run(self):
		kodi_utils.logger('Patriot Light', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('Patriot Light', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else:
					if status in ('success', 'no account'): kodi_utils.logger('Patriot Light', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else: kodi_utils.logger('Patriot Light', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('patriotlight.trakt.refresh_widgets', 'false') == 'true': kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('Patriot Light', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return kodi_utils.logger('Patriot Light', 'TraktMonitor Service Finished')

class UpdateCheck:
	def run(self):
		if kodi_utils.get_property(firstrun_update_prop) == 'true': return
		kodi_utils.logger('Patriot Light', 'UpdateCheck Service Starting')
		...
		return kodi_utils.logger('Patriot Light', 'UpdateCheck Service Finished')

class WidgetRefresher:
	def run(self):
		kodi_utils.logger('Patriot Light', 'WidgetRefresher Service Starting')
		from time import time
		...
		while not monitor.abortRequested():
			try:
				wait_for_abort(10)
				offset = int(get_setting('patriotlight.widget_refresh_timer', '60'))
				if offset != self.offset:
					self.set_next_refresh(time())
					continue
				if self.condition_check(): continue
				if self.next_refresh < time():
					kodi_utils.logger('Patriot Light', 'WidgetRefresher Service - Widgets Refreshed')
		...
		return kodi_utils.logger('Patriot Light', 'WidgetRefresher Service Finished')
	...
	def condition_check(self):
		...
		if kodi_utils.get_property('patriotlight.window_loaded') == 'true': return True 
		try:
			window_stack = json.loads(kodi_utils.get_property('patriotlight.window_stack'))
			if window_stack or window_stack == []: return True
		except: pass
		return False

	def set_next_refresh(self, _time):
		self.offset = int(get_setting('patriotlight.widget_refresh_timer', '60'))
		if self.offset: self.next_refresh = _time + (self.offset*60)
		else: self.next_refresh = None

class AutoStart:
	def run(self):
		kodi_utils.logger('Patriot Light', 'AutoStart Service Starting')
		from modules.settings import auto_start_patriotlight
		if auto_start_patriotlight(): kodi_utils.run_addon()
		return kodi_utils.logger('Patriot Light', 'AutoStart Service Finished')

class AddonXMLCheck:
	def run(self):
		kodi_utils.logger('Patriot Light', 'AddonXMLCheck Service Starting')
		from xml.dom.minidom import parse as mdParse
		self.addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.patriotlight/addon.xml')
		...
		return kodi_utils.logger('Patriot Light', 'AddonXMLCheck Service Finished')

	def check_property(self, setting, tag_name):
		current_addon_setting = get_setting('patriotlight.%s' % setting, None)
	...
	def change_xml_file(self):
		...
		kodi_utils.logger('Patriot Light', 'AddonXMLCheck Service - Change Detected. Restarting Addons')
		kodi_utils.execute_builtin('ActivateWindow(Home)', True)
		kodi_utils.update_local_addons()
		kodi_utils.disable_enable_addon()


class PatriotLightMonitor(Monitor):
	def __init__ (self):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			kodi_utils.logger('OnNotificationActions', 'PAUSING Patriot Light Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			kodi_utils.clear_property(pause_services_prop)
			kodi_utils.logger('OnNotificationActions', 'UNPAUSING Patriot Light Services Due to Device Awake')

kodi_utils.logger('Patriot Light', 'Main Monitor Service Starting')
PatriotLightMonitor().waitForAbort()
kodi_utils.logger('Patriot Light', 'Main Monitor Service Finished')
