...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'Patriot Light')
	...
	def tools(self):
		...
		if get_setting('patriotlight.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('patriotlight.use_viewtypes', 'true') == 'true' and not get_setting('patriotlight.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		patriotlight_clogpath = k.translate_path('special://home/addons/plugin.video.patriotlight/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': patriotlight_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def exit_media_menu(self):
		params = k.get_property('patriotlight.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.patriotlight/resources/text/tips'
