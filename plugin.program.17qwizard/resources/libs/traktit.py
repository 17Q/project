...
ORDER		  = ['htpctv', 'david', 'exegesis', 'patriot', 'scrubsv2', 'venom', 'judaea', 'seren', 'fen','exodusredux', 'metaq', 'trakt']
...
TRAKTID = { 
	...
	'patriot': {
		...
		'file'	 : os.path.join(TRAKTFOLD, 'patriot_trakt'),
		'settings' : os.path.join(ADDOND, 'plugin.video.patriot', 'settings.xml'),
		'default'  : 'trakt.user',
		'data'	 : ['trakt.user', 'trakt.refresh', 'trakt.token'],
		'activate' : 'RunPlugin(plugin://plugin.video.patriot/?action=authTrakt)'},
	'scrubsv2': {
		'name'	 : 'Scrubs v2',
